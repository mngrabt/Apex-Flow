...
BEGIN
    ...
    FOR v_supplier IN 
        WITH matching_suppliers AS (
            ...
            FROM users u
            JOIN database_suppliers ds ON LOWER(TRIM(ds.name)) = LOWER(TRIM(u.name))
        ...
        SELECT *
        FROM matching_suppliers
        WHERE user_notifications_enabled = true
        AND supplier_notifications_enabled = true
    LOOP
        -- Log supplier match
        INSERT INTO debug_logs (function_name, message, details)
        VALUES (
