...
import { ArrowLeft, Building2, Download, FileText, User2 } from 'lucide-react';
...
export default function ProtocolDetails() {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      // Get updated protocol data
      await fetchProtocols();
      const updatedProtocol = protocols.find(p => p.id === protocolData.id);
      
      if (updatedProtocol) {
        const signedUserIds = updatedProtocol.signatures.map(sig => sig.userId);
        const allSigned = requiredSignatures.every(id => signedUserIds.includes(id));
